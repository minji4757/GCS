var Uav = function () {
    ...
    this.drawUav = function () {
        try {
            ...
            jsproxy.log("ERROR", "uav.drawing()", err);
        }
    };
    ...
    this.drawDestinationLine = function () {
        try {
            ...
            //비행 모드별 목표 위치 설정
            this.destinationLocation = this.currLocation; //land
    ...
    this.setHomePosition = function (lat, lng) {
        if (this.homePosition == null || this.homePosition.lat != lat || this.homePosition.lng != lng) {
            try {
                //홈 위치 저장
    ...
    this.manualMove = function (manualLat, manulLng) {
        try {
            //비행 모드 설정
            ...
            this.manualTargetMarker = new google.maps.Marker({
                map: map.googlemap,
                position: {lat: manualLat, lng: manulLng},
                optimized: false
            });
            //드론에게 클릭지점을 전달
            jsproxy.manualMove(manualLat, manulLng, this.manualAlt);
        } catch (err) {
            jsproxy.log("ERROR", "uav.manualMove()", err);
        }
    };
    //비행 모드별 필드와 비행 모드 변경 메소드
    ...
    this.setMode = function (guidedMode, autoMode, rtlMode, landMode) {
        ...
        if(guidedMode == false) {
            if (this.manualTargetMarker !=null) {
                this.manualTargetMarker.setMap(null); //가다가 복귀하면 목표지점이 없어진다.
            }
        }
    };

    //미션 필드와 메소드
    this.missionMarkers = []; //자바스크립트 배열 길이는 가변 =자바의 리스트
    this.missionMarkerMake = function(missionItem) {
        try {
            //새로운 MissionItem Marker 생성
            var marker = new google.maps.Marker({
                map: map.googlemap,
                position: {lat: missionItem.x, lng: missionItem.y},
                optimized: false,
                icon : {
                    path: google.maps.SymbolPath.CIRCLE, //구글에서 가져다 씀.
                    fillOpacity:1, //불투명
                    fillColor: "#ffffff",
                    strokeColor: "#00ffff", //테두리
                    strokeWeight: 1,
                    scale: 12
                },
                label: { //시퀀스
                    color: "#000000",
                    fontSize: "12px",
                    fontWeight: "600", //아이콘 크기보다 더 커지면 디폴트 값이 됌.
                    text: String(missionItem.seq)
                }
            });
            //marker.setDraggable(true) //지도상에서 마커 드래그 이동 가능.
    ...
    this.drawMissionPath = function () {
      try {
          ...
          var endMarker = null;
          //미션선 생성
          for(var i = 1; i < this.missionMarkers.length; i++) { //0이미 지정
